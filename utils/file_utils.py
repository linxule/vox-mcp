    ...
    try:
        ...
        for pattern in home_patterns:
            if pattern in path_str:
                ...
                # e.g., /Users/someone or /home/username
...
def resolve_and_validate_path(path_str: str) -> Path:
    ...
    if not user_path.is_absolute():
        raise ValueError(f"Relative paths are not supported. Use an absolute path.\nReceived: {path_str}")
...
def check_total_file_size(files: list[str], model_name: str) -> dict | None:
    ...
    This performs strict rejection at the MCP boundary.
