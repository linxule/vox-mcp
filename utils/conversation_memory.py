...
⚠️  This system will fail if MCP tool calls are made
    ...
    FAILING SCENARIO: one-shot subprocess harnesses that invoke server.py per request
...
A conversation started with one tool (e.g., 'chat') can be continued with
any other tool (e.g., 'listmodels', 'dump_threads') using the same continuation_id.
...
- Automatic turn limiting (configurable, default 100) to prevent runaway conversations
- Context reconstruction for stateless request continuity
- In-memory persistence with automatic expiration (configurable, default 24h TTL)
...
USAGE EXAMPLE:
1. Tool A creates thread: create_thread("chat", request_data) → returns UUID
2. Tool A adds response: add_turn(UUID, "assistant", response, files=[...], tool_name="chat")
...
5. Tool B adds its response: add_turn(UUID, "assistant", response, tool_name="dump_threads")
...
# Get max conversation turns from environment, default to 100 turns (50 exchanges)
try:
    max_turns_raw = (get_env("MAX_CONVERSATION_TURNS", "100") or "100").strip()
    MAX_CONVERSATION_TURNS = int(max_turns_raw)
    if MAX_CONVERSATION_TURNS <= 0:
        logger.warning(f"Invalid MAX_CONVERSATION_TURNS value ({MAX_CONVERSATION_TURNS}), using default of 100 turns")
        MAX_CONVERSATION_TURNS = 100
except ValueError:
    logger.warning(
        f"Invalid MAX_CONVERSATION_TURNS value ('{get_env('MAX_CONVERSATION_TURNS')}'), using default of 100 turns"
    )
    MAX_CONVERSATION_TURNS = 100

# Get conversation timeout from environment (in hours), default to 24 hours
try:
    timeout_raw = (get_env("CONVERSATION_TIMEOUT_HOURS", "24") or "24").strip()
    CONVERSATION_TIMEOUT_HOURS = int(timeout_raw)
    if CONVERSATION_TIMEOUT_HOURS <= 0:
        logger.warning(
            f"Invalid CONVERSATION_TIMEOUT_HOURS value ({CONVERSATION_TIMEOUT_HOURS}), using default of 24 hours"
        )
        CONVERSATION_TIMEOUT_HOURS = 24
except ValueError:
    logger.warning(
        f"Invalid CONVERSATION_TIMEOUT_HOURS value ('{get_env('CONVERSATION_TIMEOUT_HOURS')}'), using default of 24 hours"
    )
    CONVERSATION_TIMEOUT_HOURS = 24
...
def create_thread(tool_name: str, initial_request: dict[str, Any], parent_thread_id: str | None = None) -> str:
    ...
    Note:
        - Thread expires after the configured timeout (default: 24 hours)
    ...
    # Populate client identity from MCP handshake cache
    client_name = None
    client_version = None
    try:
    ...
    # Shadow-persist to disk
    try:
        from utils.thread_persistence import save_thread_header

        save_thread_header(context)
    except Exception as e:
        logger.debug(f"[THREAD] Disk persistence failed for header: {e}")
...
def get_thread(thread_id: str) -> ThreadContext | None:
    ...
    try:
        ...
        # Cold-reload from disk on cache miss
        try:
            from utils.thread_persistence import load_thread_from_disk

            context = load_thread_from_disk(thread_id)
            if context:
                # Re-populate memory with fresh TTL
                storage.setex(key, CONVERSATION_TIMEOUT_SECONDS, context.model_dump_json())
                logger.info(f"[THREAD] Cold-reloaded thread {thread_id[:8]} from disk")
                return context
        except Exception as e:
            logger.debug(f"[THREAD] Disk cold-reload failed: {e}")
...
def add_turn(
    ...
    # Shadow-persist turn to disk
    try:
        from utils.thread_persistence import append_turn as persist_turn

        persist_turn(thread_id, context.created_at, turn)
    except Exception as e:
        logger.debug(f"[FLOW] Disk persistence failed for turn: {e}")
...
def build_conversation_history(context: ThreadContext, model_context=None, read_files_func=None) -> tuple[str, int]:
    ...
    Cross-Tool Collaboration:
        ...
        file contents from previous tools, enabling seamless handoffs between chat,
        listmodels, dump_threads, and other tools while maintaining complete context.
    ...
    history_parts.extend(
        [
            ...
            "=== END CONVERSATION HISTORY ===",
            "",
            f"Next turn index: {len(all_turns) + 1}",
        ]
    )
