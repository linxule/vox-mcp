...
class InMemoryStorage:
    ...
    def __init__(self):
        ...
        # Run cleanup at 1/10th of timeout interval (e.g., ~2.4 hrs for 24 hour timeout)
    ...
    def dump_all(self) -> dict[str, str]:
        """Dump all non-expired entries. Returns {key: value_json_string}."""
        with self._lock:
            current_time = time.time()
            return {k: v for k, (v, exp) in self._store.items() if exp > current_time}
