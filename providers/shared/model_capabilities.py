...
from .thinking import ThinkingConstraint
...
class ModelCapabilities:
    ...
    def get_effective_thinking_params(self, thinking_mode: str | None = None) -> dict[str, Any] | None:
        ...
        configured).  Returns an empty dict for always-on thinking that
        needs no additional API parameters.  Otherwise returns a dict of
        parameters the provider should merge into its request payload.
        """
        if self.thinking_constraint is None:
            return None

        mode = thinking_mode or self.thinking_constraint.get_default_mode()
        return self.thinking_constraint.resolve(mode)
