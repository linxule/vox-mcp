"""
Core Tool Infrastructure for Vox MCP Tools
...
class BaseTool(ABC):
    """
    Abstract base class for all Vox MCP tools.
    ...
    1. FILE DEDUPLICATION WITH NEWEST-FIRST PRIORITY:
       ...
       - Cross-tool file tracking ensures consistent behavior across tool handoffs
    ...
    def get_name(self) -> str:
        ...
        Returns:
            str: The tool's unique name (e.g., "chat", "listmodels")
        """
        pass
    ...
    def get_system_prompt(self) -> str:
        """
        Return the tool's optional system prompt.

        Prompt passthrough mode returns an empty string by default.

        Returns:
            str: Optional system prompt text
        """
        return ""
    ...
    def requires_model(self) -> bool:
        ...
        Tools that override execute() to do pure data processing
    ...
    def _build_model_unavailable_message(self, model_name: str) -> str:
        ...
        return (
            ...
            f"(category: {tool_category.value}). If the user explicitly requested a model, use that exact name or report this error back without substitution."
        )
    ...
    def get_model_field_schema(self) -> dict[str, Any]:
        ...
        if self.is_effective_auto_mode():
            description = (
                "Currently in auto model selection mode. "
                "If no model is provided, you may use the `listmodels` tool to review options and select an appropriate match. "
                "The server validates model availability and returns errors for unknown models."
            )
        ...
        description = (
            f"Model name override. Defaults to '{DEFAULT_MODEL}' if omitted. "
            "Use the `listmodels` tool for available options. "
            "The server validates model availability and returns errors for unknown models."
        )
        summaries, total, restricted = self._get_ranked_model_summaries()
        remainder = max(0, total - len(summaries))
    ...
    def check_prompt_size(self, text: str) -> Optional[dict[str, Any]]:
        ...
        This method validates user input that crosses the CLI ↔ MCP Server
        transport boundary. It is not used to limit internal MCP operations.
        ...
        if text and len(text) > MCP_PROMPT_SIZE_LIMIT:
            return {
                ...
                "content": (
                    f"The prompt exceeds MCP transport limits (>{MCP_PROMPT_SIZE_LIMIT:,} characters). "
                    "Save the full prompt text to 'prompt.txt' in the working directory and resend the request "
                    "with the absolute path to that file. Keep the prompt field empty or short on resend."
                ),
                "content_type": "text",
                "metadata": {
                    "prompt_size": len(text),
                    "limit": MCP_PROMPT_SIZE_LIMIT,
                    "instructions": "Save prompt to 'prompt.txt' and include its absolute path on resend.",
                },
            }
        return None
    ...
    def _resolve_model_context(self, arguments: dict, request) -> tuple[str, Any]:
        ...
        reused by tools that override execute() without duplicating code.
