...
class SimpleTool(BaseTool):
    """
    Base class for single-request tools.

    Simple tools are request/response tools that don't require multi-step coordination.
    They benefit from:
    ...
    async def execute(self, arguments: dict[str, Any]) -> list:
        ...
        try:
            ...
            # Prompt passthrough mode: no system prompt is injected.
            system_prompt = None
            ...
            if model_response.content:
                ...
                if metadata.get("is_blocked_by_safety"):
                    ...
                    if finish_reason == "STOP":
                        # Model completed normally but returned empty content
                        logger.info(
                            "Model completed with empty response for %s (finish_reason=STOP)",
                            self.get_name(),
                        )
                        tool_output = ToolOutput(
                            status="error",
    ...
    def build_standard_prompt(
        ...
        """
        Build a prompt from user content and optional file context.
        ...
        Args:
            system_prompt: Unused legacy parameter kept for compatibility
            user_content: The main user request/content
            request: The validated request object
            file_context_title: Title for the file context section

        Returns:
            Prompt content ready for the AI model
        """
        _ = system_prompt
    ...
    def prepare_chat_style_prompt(self, request, system_prompt: str = None) -> str:
        """
        Prepare a prompt using the shared prompt/file pipeline.

        Args:
            request: The validated request object
            system_prompt: Optional legacy parameter
        ...
        return self.build_standard_prompt(system_prompt, user_content, request, "CONTEXT FILES")
