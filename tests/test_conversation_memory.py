...
class TestConversationMemory:
    ...
    @patch("utils.thread_persistence.load_thread_from_disk", return_value=None)
    @patch("utils.conversation_memory.get_storage")
    def test_get_thread_not_found(self, mock_storage, mock_load_disk):
        """Test handling thread not found (memory miss + disk miss)"""
        mock_client = Mock()
        mock_storage.return_value = mock_client
    ...
    def test_build_conversation_history(self, project_path):
        ...
        # Test file tracking
        # Check that the file embedding section is included
...
class TestConversationFlow:
    ...
    def test_conversation_with_files_and_context_preservation(self, mock_storage):
        ...
        assert "--- Turn 1 (gemini-2.5-flash using chat via google) ---" in history
        assert "--- Turn 2 (Agent) ---" in history
        assert "--- Turn 3 (gemini-2.5-flash using chat via google) ---" in history
        ...
        turn_1_pos = history.find("--- Turn 1 (gemini-2.5-flash using chat via google) ---")
        turn_2_pos = history.find("--- Turn 2 (Agent) ---")
        turn_3_pos = history.find("--- Turn 3 (gemini-2.5-flash using chat via google) ---")
