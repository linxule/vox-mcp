...
class TestAutoMode:
    ...
    def test_auto_mode_detection(self):
        ...
        try:
            ...
            # Test non-auto mode
            os.environ["DEFAULT_MODEL"] = "gemini-3.1"
            importlib.reload(config)

            assert config.DEFAULT_MODEL == "gemini-3.1"
    ...
    def test_model_capabilities_descriptions(self):
        ...
        expected_models = ["flash", "gemini-3.1", "o3", "o3-mini", "o3-pro", "o4-mini"]
    ...
    def test_tool_schema_in_normal_mode(self):
        ...
        try:
            # Set to a specific model (not auto mode) — must exist in provider registry
            ...
            assert "enum" not in model_schema
            assert "listmodels" in model_schema["description"]
            assert "defaults to" in model_schema["description"].lower()
    ...
    def test_model_field_schema_generation(self):
        ...
        try:
            ...
            assert "enum" not in schema
            assert schema["type"] == "string"
            assert "'gemini-3.1'" in schema["description"]
            assert "listmodels" in schema["description"]
