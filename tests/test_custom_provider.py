...
class TestCustomProvider:
    ...
    def test_validate_model_names_always_true(self):
        ...
        # Known model from custom_models.json should validate
    ...
    def test_model_alias_resolution(self):
        ...
        # Known model from custom_models.json resolves to itself
        resolved = provider._resolve_model_name("local-llama")
        assert resolved == "local-llama"

        # Unknown model passes through as-is
        resolved_unknown = provider._resolve_model_name("some-other-model")
        assert resolved_unknown == "some-other-model"

    def test_no_thinking_mode_support(self):
        """Custom provider generic capabilities default to no thinking mode."""
        provider = CustomProvider(api_key="test-key", base_url="http://localhost:11434/v1")

        # local-llama is a known model from custom_models.json
        assert not provider.get_capabilities("local-llama").supports_extended_thinking
...
class TestCustomProviderRegistration:
    ...
    def test_provider_priority_selection(self):
        ...
        with patch.dict(
            ...
            # Custom provider only knows models from custom_models.json
            assert not custom_provider.validate_model_name("flash")
            # OpenRouter knows models from openrouter_models.json (e.g., "flash" alias)
            assert openrouter_provider.validate_model_name("flash")
